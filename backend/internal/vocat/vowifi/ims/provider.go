...
type Config struct {
	// Called only when remote deregistration failed but all local resources were
	// released. Opt-in deployments may warn instead of blocking local teardown.
	OnDeregistrationUnconfirmed func()
...
func (session *Session) Close(ctx context.Context) error {
	...
	if session.provider != nil {
		notify = session.provider.config.OnDeregistrationUnconfirmed
	}
	return closeResult(unregisterErr, cleanupErrors, notify)
}

func closeResult(remote error, local []error, notify func()) error {
	localErr := errors.Join(local...)
	if localErr != nil {
		return errors.Join(remote, localErr)
	}
	if remote != nil && notify != nil {
		notify()
		return nil
	}
	return remote
