...
Go + PostgreSQL。实现登录、会话恢复、退出、管理员修改、独立密码、功能显示配置与 Cloudflare Tunnel，新增模块自动发现、状态读取、唯一标签及 eSIM 配置管理接口；硬件适配待实机联调。网络搜索及自动、手动选网已接入，EC20 实机搜索通过；实际切换另行验证。短信、SIP、Wi-Fi 通话和数据漫游控制尚未接通。详见 [模块读取](MODULES.md)。
