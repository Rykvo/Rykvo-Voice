...
Go + PostgreSQL。实现登录、会话、管理员改密、独立密码、功能显示、Cloudflare Tunnel、模块发现和 eSIM 管理。Wi-Fi 通话已接入 EC20 的飞行模式联动、IMS 注册及续期；实际呼叫/音频、短信和数据漫游仍待实现。手动搜网与选网已移除。详见 [模块读取](MODULES.md)。
