...
已接通：登录、管理员改密、独立密码、功能显示和 Cloudflare Tunnel。新增模块自动发现、状态读取、唯一标签及 eSIM 配置管理代码，待实机联调。手动搜网与选网已移除，网络状态只读采集。**Wi-Fi 通话、数据连接及漫游控制、短信、SIP、任务队列和图片存储仍待实现**；不安装空转的 RabbitMQ / S3 服务，也不把环境安装当作业务已完成。
