...
已接通：登录、管理员改密、独立密码、功能显示和 Cloudflare Tunnel。新增模块自动发现、状态读取、唯一标签及 eSIM 配置管理代码，待实机联调。网络搜索及自动、手动选网已接入，EC20 实机搜索通过，实际切换待验证。**Wi-Fi 通话、数据连接及漫游控制、短信、SIP、任务队列和图片存储仍待实现**；不安装空转的 RabbitMQ / S3 服务，也不把环境安装当作业务已完成。
...
QMI 状态通过应用专用的本机套接字读取。发行版 qmi-proxy 可能只接受 root，设备节点的组权限不能替代代理授权；因此使用按需启动的只读查询进程，后端仍为普通服务账号。入口仅允许固定的身份、SIM、注册状态和信号查询，不接受搜索、选网、复位或任意命令；无 TCP 端口，查询超时自动结束。套接字及服务随安装、备份、回退和卸载处理。

AT 暂无响应时由 QMI 读取基础状态，保留 AT 错误码供诊断；不自动重发选网或复位模块。注册被拒绝与读取失败分别显示。QMI 基础读取恢复不代表 AT 控制、eSIM 操作或漫游注册已恢复。
