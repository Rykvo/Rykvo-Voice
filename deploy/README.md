...
每次更新必须完成源码、Latest 安装包与主机三方同步。增加 `VERSION`、更新 `deploy/RELEASE_NOTES.md` 后推送同名 `vX.Y.Z` 标签；标签的 Check 工作流通过全部测试后，会自动发布两种架构安装包及校验清单。随后从该 Release 更新主机并核对实际运行文件与模块状态。CI 不持有主机 SSH 凭据，不会自行修改主机。

只推送 main 或只热更新主机都不算发布完成。不要覆盖旧版本资产。
...
QMI 状态通过应用专用的本机套接字读取。发行版 qmi-proxy 可能只接受 root，设备节点的组权限不能替代代理授权；因此使用按需启动的固定设备命令进程，后端仍为普通服务账号。入口允许固定的身份、SIM、注册状态和信号查询，以及核对 USB 端点和设备世代后的 EC20 单模块恢复；不接受搜索、选网、任意命令或整板复位；无 TCP 端口，查询超时自动结束。套接字及服务随安装、备份、回退和卸载处理。

AT 暂无响应时由 QMI 读取基础状态，保留 AT 错误码供诊断；不自动重发选网。连续通信超时的恢复规则见模块说明。注册被拒绝与读取失败分别显示。QMI 基础读取恢复不代表 AT 控制、eSIM 操作或漫游注册已恢复。

模块能力与验证边界见 [模块读取](../backend/MODULES.md)。生产虚拟机是否透传 USB 需要单独检查，更新程序不会自动配置宿主机 USB 透传。

### 重启操作
模块页支持独立模块、全部 USB 模块和主机重启，均需二次确认。现有私有 QMI 辅助服务增加固定的延迟主机重启操作，不接受任意命令。忙碌设备不强制中断，重启请求不自动重放。
